if _G.YOKANA_SECRET_AUTH ~= "T0x1c_L04d3r_V2" then return end
_G.YOKANA_SECRET_AUTH = nil 
...
function playAlertSound(x, y, z) pcall(function() addOneOffSound(x, y, z, 1058) end) end
...
function repairEngine()
    if isCharInAnyCar(PLAYER_PED) then
        ...
        if doesVehicleExist(car) then
            ...
            sampAddChatMessage("{9932CC}[YoVehicle]{FFFFFF} Mesin udah mulus lagi.", -1)
        end
    end
end
...
function repairBody()
    if isCharInAnyCar(PLAYER_PED) then
        ...
        if doesVehicleExist(car) then
            ...
            setCarHealth(car, 1000.0)
            lastVehHealth = 1000.0
            sampAddChatMessage("{9932CC}[YoVehicle]{FFFFFF} Bodi udah diketok magic.", -1)
        end
    end
end
...
function doScan(isManual)
            ...
            if res and text then
                ...
                if txtLower:find("tree") then
                    ...
                    if dist <= scanRadius[0] then
                        ...
                        if isTumbang then
                            if showDown[0] then table.insert(temp, { type = "TUMBANG", num = treeNum, info = timeMatch or clean:gsub("\n", " "), x = x, y = y, z = z, dist = dist }) end
                        else
    ...
    for k, _ in pairs(notifiedTrees) do if not currentReadyNumMap[k] then notifiedTrees[k] = nil end end
    table.sort(temp, function(a, b) return a.dist < b.dist end)
    readyTrees = temp
    if autoGPS[0] then
        for _, t in ipairs(readyTrees) do if t.type == "TEBANG" then setGPS(t.x, t.y, t.z) break end end
    end
end
...
lua_thread.create(function()
    ...
    sampAddChatMessage("{9932CC}[YoLumber V2]{FFFFFF} Script by Yokana aktif via Cloud! Ketik {9932CC}/yolumber{FFFFFF} buat buka menu.", -1)
    ...
    lua_thread.create(function()
        while true do
            ...
            if isPlayerPlaying(PLAYER_HANDLE) and autoEatDrink[0] then
                sampAddChatMessage("{9932CC}[YoLumber V2]{FFFFFF} Waktunya makan minum! Nyuapin lu 5x Snack & 5x Sprunk...", -1)
    ...
    lua_thread.create(function()
        while true do
            ...
            if autoUnflip[0] and isCharInAnyCar(PLAYER_PED) then
                ...
                if not flyingVehicles[model] and isCarUpsidedown(veh) then
                    wait(1000) 
                    if isCharInAnyCar(PLAYER_PED) then
...
imgui.OnFrame(function() 
    ...
    if espEnabled[0] and #readyTrees > 0 then
        for i, t in ipairs(readyTrees) do
            ...
            if isPointOnScreen(t.x, t.y, t.z, 0.0) then
            ...
            if sampIsPlayerConnected(i) then
                ...
                if result and doesCharExist(ped) and not isCharDead(ped) then
                    ...
                    if weaponNames[weaponId] then
                        ...
                        if dist <= weaponEspDistance[0] and isPointOnScreen(cx, cy, cz, 0.0) then
                            ...
                            if onScreen and sx and sy then
                                ...
                                local strWeap = string.format("[AWAS DM] %s | %s (ID: %d) [%.1fm]", pName, weaponNames[weaponId], weaponId, dist)
...
imgui.OnFrame(function() return windowState[0] end, function(player)
    ...
    imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1.0), u8("Developed by: Yokana | Edisi Kuli Kasar (API Cloud)"))
    imgui.Separator()
    imgui.Spacing()
    ...
    if imgui.Button(u8"[ INFO ] JOIN SALURAN", imgui.ImVec2(-1, 40)) then openLink("https://whatsapp.com/channel/0029VbDrYKWLikg0DjWKi82X") end
    imgui.PopStyleColor(3) 
    imgui.Spacing()
    ...
    if currentTab[0] == 1 then 
        if imgui.Button(u8"REFRESH RADAR SEKARANG (BIAR UPDATE)", imgui.ImVec2(-1, 45)) then doScan(true) end
        imgui.Spacing()
        ...
        if #readyTrees == 0 then
            ...
            for i, t in ipairs(readyTrees) do
                ...
                if imgui.Button(u8("GPS##" .. i), imgui.ImVec2(60, 30)) then setGPS(t.x, t.y, t.z) end
                imgui.SameLine()
                local col = (t.type == "TEBANG") and imgui.ImVec4(1.0, 1.0, 1.0, 1.0) or imgui.ImVec4(1.0, 0.3, 0.3, 1.0)
                imgui.TextColored(col, u8(string.format("%s Pohon (%s) - %.1fm", tag, t.num, t.dist)))
            end
        end
        imgui.EndChild()
    ...
    elseif currentTab[0] == 3 then 
        ...
        imgui.PopItemWidth()
        if imgui.Button(u8("BENERIN MESIN"), imgui.ImVec2(-1, 50)) then repairEngine() end
        if imgui.Button(u8("KETOK MAGIC BODI"), imgui.ImVec2(-1, 50)) then repairBody() end
    elseif currentTab[0] == 4 then 
        if imgui.Button(u8("Tebang Pohon"), imgui.ImVec2(-1, 50)) then sampSendChat("/lum cut") end
        if imgui.Button(u8("Ambil Kayu Jatuh"), imgui.ImVec2(-1, 50)) then sampSendChat("/lum take") end
        if imgui.Button(u8("Muat ke Mobil"), imgui.ImVec2(-1, 50)) then sampSendChat("/lum load") end
        if imgui.Button(u8("Turunkan Kayu"), imgui.ImVec2(-1, 50)) then sampSendChat("/lum unload") end
        if imgui.Button(u8("Potong Log"), imgui.ImVec2(-1, 50)) then sampSendChat("/lum slice") end
    end
    imgui.End()
